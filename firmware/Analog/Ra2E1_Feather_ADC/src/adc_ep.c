...
static uint16_t g_adc_data1;
static uint16_t g_adc_data2;
static uint16_t g_adc_data3;
static uint16_t g_adc_data4;
static uint16_t g_adc_data5;
...
fsp_err_t adc_read_data(void)
{
    ...
    err = R_ADC_Read (&g_adc_ctrl, ADC_CHANNEL_1, &g_adc_data1);
    err = R_ADC_Read (&g_adc_ctrl, ADC_CHANNEL_2, &g_adc_data2);
    err = R_ADC_Read (&g_adc_ctrl, ADC_CHANNEL_7, &g_adc_data3);
    err = R_ADC_Read (&g_adc_ctrl, ADC_CHANNEL_8, &g_adc_data4);
    err = R_ADC_Read (&g_adc_ctrl, ADC_CHANNEL_9, &g_adc_data5);
    /* handle error */
    if (FSP_SUCCESS != err)
    {
        /* ADC Failure message */
...
//    APP_PRINT("\r\nThe ADC input voltage: %s\r\n", volt_str);
    ...
    adc_volt = (float)((g_adc_data * V_ref)/ADC_12_BIT);
    snprintf((char *)volt_str,SIZE_64, "%0.2f", adc_volt);
    APP_PRINT("\r\nThe ADC input voltage A0: %s\r\n", volt_str);

    adc_volt = (float)((g_adc_data1 * V_ref)/ADC_12_BIT);
    snprintf((char *)volt_str,SIZE_64, "%0.2f", adc_volt);
    APP_PRINT("\r\nThe ADC input voltage A1: %s\r\n", volt_str);

    adc_volt = (float)((g_adc_data2 * V_ref)/ADC_12_BIT);
    snprintf((char *)volt_str,SIZE_64, "%0.2f", adc_volt);
    APP_PRINT("\r\nThe ADC input voltage A2: %s\r\n", volt_str);

    adc_volt = (float)((g_adc_data3 * V_ref)/ADC_12_BIT);
    snprintf((char *)volt_str,SIZE_64, "%0.2f", adc_volt);
    APP_PRINT("\r\nThe ADC input voltage A3: %s\r\n", volt_str);

    adc_volt = (float)((g_adc_data4 * V_ref)/ADC_12_BIT);
    snprintf((char *)volt_str,SIZE_64, "%0.2f", adc_volt);
    APP_PRINT("\r\nThe ADC input voltage A4: %s\r\n", volt_str);

    adc_volt = (float)((g_adc_data5* V_ref)/ADC_12_BIT);
    snprintf((char *)volt_str,SIZE_64, "%0.2f", adc_volt);
    APP_PRINT("\r\nThe ADC input voltage A5: %s\r\n", volt_str);

...
void init_sensor(void)
{
    ...
    if (FSP_SUCCESS != err)
    {
        Serial1.print((uint8_t *)"** R_IIC_MASTER_Open API failed ** \r\n");
        i2cok = false;
        //APP_ERR_TRAP(err);
    }
    ...
    if ( (FSP_SUCCESS == err) && (DEVICE_SIGNATURE == device_id) )
    {
        ...
        else
        {
            ...
            Serial1.print((uint8_t *)"** R_IIC_MASTER_Write API failed ** \r\n");
            i2cok = false;
            //APP_ERR_TRAP(err);
        }
    }
...
void read_sensor_data(uint8_t *xyz_data)
{
    ...
    err = R_IIC_MASTER_Write(&g_i2c_master0_ctrl, &internal_reg, ONE_BYTE, true);
    /* handle error */
    if (FSP_SUCCESS != err)
    {
        Serial1.print((uint8_t *)"** R_IIC_MASTER_Write API failed ** \r\n");
    ...
    else
    {
        ...
        if (FSP_SUCCESS == err)
        {
            err = R_IIC_MASTER_Read(&g_i2c_master0_ctrl, &adxl_data[0], DATA_REGISTERS, false);
            /* handle error */
            if (FSP_SUCCESS != err)
            {
                Serial1.print((uint8_t*)"** R_IIC_MASTER_Read API failed ** \r\n");
                i2cok = false;
                //APP_ERR_TRAP(err);
                return;
            }
